...
if ( checkArray($_POST,['buy'], ['idCC']) and checkIfEmptyArray($_POST,['idCC'])) {
    ...
    $dataResp = acheterCC($odb, $_SESSION['id'],$data['idCC']);
...
if ( isset($_GET['ccnum'])) {
    $_GET['ccnum'] = str_replace(" ","",$_GET['ccnum']);
    $_GET['ccnum'] = '['.implode(',',explode("\n",$_GET['ccnum'])) . ']';
}
$_GET['user']= '0';
$_GET['dob'] = substr(isset($_GET['dob'])?$_GET['dob']:'',0,4);

$cards = getData($odb, 'cards',$_GET,isset($_GET['orderColumn'])?$_GET['orderColumn']:(isset($_GET['page'])?null:'id'),isset($_GET['order'])?$_GET['order']:'DESC');
$ccTotal = getCount($odb, 'cards',$_GET);

if(empty($ccTotal))
	$ccTotal = 0;

if ( isset($_GET['ccnum'])) {
    $_GET['ccnum'] = str_replace(",", "", str_replace("]", "", str_replace("[", "", $_GET['ccnum'])));
}
...
?>
				 ...
				 <div class="card-header card-header-large bg-white d-flex align-items-center">
                                        <h4 class="card-header__title flex m-0">Cartes bancaire disponible <span class="badge badge-soft-primary badge-pill mr-1"><?php echo $ccTotal; ?></span><br>
										<small class="text-muted">Parcours nos récentes cartes bancaire et choisis celles qui te plaisent. Car nous on va chercher l'argent la ou elle est.. C'est-à-dire dans les banques!</small></h4>
...
<?php foreach ($cards as $cc ) { ?>
                                        <tr><form action="" method="POST">
											...
											<?php echo substr($cc["ccnum"], 0, 6); ?></span></td>
                                            <td><?php echo $cc["ccexp"]; ?></td>
                                            <td><?php echo $cc["level"]; ?></td>
                                            <td><?php echo $cc["type"]; ?></td>
                                            <td><small class="text-muted"><?php echo $cc["banque"]; ?></small></td>
                ...
                <div class="col-lg-12 card-form_body card-body">
                 <div class="form-row">
                        ...
                        <div class="col-6 mb-3">
                             <label class="mesrine">BIN</label>
                
                        <textarea name="ccnum" class="form-control tx-color-03" placeholder="1 Bin par ligne..."><?php echo isset($_GET['ccNum'])?$_GET['ccNum']:'' ?></textarea>
                  </div><!-- col -->
				  
				  <div class="col-6 mb-3">
                             <label class="mesrine">Banque</label>
                     <select name="banque" class="form-control tx-color-03">
                         <option value="">Toutes</option>
                         <?php foreach(getData($odb, 'cards',['user'=>'0'],null,null,'banque') as $banque) {
                             echo '<option '.(isset($_GET['banque']) && $_GET['banque'] == $banque['banque']?'selected':'').' value="'.$banque['banque'].'">'.$banque['banque'].'</option>';
                         } ?>
					 </select>
					 </div>
                  </div><!-- col -->
				 ...
				 <div class="form-row">
                        <div class="col-6 mb-3">
                             <label class="mesrine">Level</label>
                     <select name="level" class="form-control tx-color-03">
                         <option value="">Tous</option>
                        <?php foreach(getDataWithoutPage($odb, 'cards',['user'=>'0'],null,null,'level') as $type) {
                            echo '<option '.(isset($_GET['level']) && $_GET['level'] == $type['level']?'selected':'').' value="'.$type['level'].'">'.$type['level'].'</option>';
                        } ?>
					 </select></div>
                  <div class="col-6 mb-3">
                    ...
                    <select name="type" class="form-control tx-color-03">
                        <option value="">Tous</option>
                        <?php foreach(getDataWithoutPage($odb, 'cards',['user'=>'0'],null,null,'type') as $type) {
                            echo '<option '.(isset($_GET['type']) && $_GET['type'] == $type['type']?'selected':'').' value="'.$type['type'].'">'.$type['type'].'</option>';
                        } ?>
					 </select></div>
                  </div><!-- col -->
				  ...
				  <div class="form-row">
                     ...
                     <select name="pays" class="form-control tx-color-03">
                        <option value="">Tous</option>
                        <?php foreach(getDataWithoutPage($odb, 'cards',['user'=>'0'],null,null,'pays') as $country) {
				   ...
				   <div class="col-6 mb-3">
                     ...
                     <select name="base" class="form-control tx-color-03">
                        <option value="">Toutes</option>
                        <?php foreach(getDataWithoutPage($odb, 'cards',['user'=>'0'],null,null,'base') as $country) {
                        ...
                        <div class="col-6 mb-3">
                             <label class="mesrine">Cartes par page</label>
